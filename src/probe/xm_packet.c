...
void xm_dump_ip_header(FILE *fp, struct ip *iph)
{
...
void xm_dump_eth_header(FILE *fp, struct ether_header *ethh)
{
		fprintf(fp,
...
void xm_make_eth_header(struct ether_header *ethh, uint8_t *src, uint8_t *dst)
{
...
void xm_make_ip_header(struct ip *iph, uint8_t protocol, uint16_t len)
{
...
void xm_make_icmp_header(struct icmp *buf)
{
...
void xm_make_tcp_header(struct tcphdr *tcp_header, uint16_t dest_port,
		     uint16_t th_flags)
{
...
void xm_make_udp_header(struct udphdr *udp_header, uint16_t dest_port,
		     uint16_t len)
{
