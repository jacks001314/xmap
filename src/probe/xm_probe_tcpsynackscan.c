 ...
 * =====================================================================================
 *
 *       Filename:  xm_probe_tcpsynackscan.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  11/20/2019 10:21:23 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
static int synackscan_probe_context_init(xm_context_t *context){
...
static int synackscan_probe_thread_init(void *pbuf,uint8_t *smac,uint8_t *dmac,uint16_t dport,void xm_unused **arg_ptr){
	...
	xm_make_tcp_header(tcp_header, dport, TH_SYN|TH_ACK);

	return EXIT_SUCCESS;

}

static int synackscan_probe_make_packet(void *pbuf,size_t xm_unused *blen,uint32_t sip,uint32_t dip,uint8_t ttl,uint32_t *validation,int probe_num,void xm_unused *arg){
...
static int synackscan_probe_validate_packet(struct ip *ip_hdr,uint32_t len,uint32_t xm_unused *sip,uint32_t *validation){
	...
	if (!xm_check_dst_port(ntohs(dport), num_ports,
                gcontext->source_port_first,
...
static void synackscan_probe_process_packet(void *pbuf,uint32_t xm_unused len,xm_store_t *xstore,uint32_t xm_unused *validation){
...
static void synackscan_probe_dump_packet(FILE *fp,void *pbuf){
...
xm_probe_t probe_tcp_synackscan = {
    .name = "tcp_synackscan",
    .packet_length = 54,
    .probe_type = PROBE_TCP_SYN,
    .port_args =1,
    .probe_context_init = synackscan_probe_context_init,
    .probe_thread_init = synackscan_probe_thread_init,
    .probe_make_packet = synackscan_probe_make_packet,
    .probe_dump_packet = synackscan_probe_dump_packet,
    .probe_validate_packet = synackscan_probe_validate_packet,
    .probe_process_packet = synackscan_probe_process_packet,
