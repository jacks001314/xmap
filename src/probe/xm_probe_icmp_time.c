 ...
 * =====================================================================================
 *
 *       Filename:  xm_probe_icmp_time.c
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  11/20/2019 09:31:13 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
static int icmp_time_probe_context_init(xm_context_t *context){
...
static int icmp_time_probe_thread_init(void *pbuf,uint8_t *smac,uint8_t *dmac,uint16_t xm_unused dport,void xm_unused **arg_ptr){
...
static int icmp_time_probe_make_packet(void *pbuf,size_t xm_unused *blen,uint32_t sip,uint32_t dip,uint8_t ttl,
...
static int icmp_time_probe_validate_packet(struct ip *ip_hdr,uint32_t len,uint32_t *sip,uint32_t *validation){
...
static void icmp_time_probe_process_packet(void *pbuf,uint32_t xm_unused len,xm_store_t *xstore, uint32_t xm_unused *  validation){
    ...
    xm_store_map_start(xstore,"icmpTime",9);
	...
	xm_store_write_uint64(xstore, "sentTimestampTS", (uint64_t)payload->sent_tv_sec);
	xm_store_write_uint64(xstore, "sentTimestampUS", (uint64_t)payload->sent_tv_usec);
	xm_store_write_uint64(xstore, "dstRaw", (uint64_t)payload->dst);

}

static void icmp_time_probe_dump_packet(FILE *fp, void *pbuf)
{
...
xm_probe_t probe_icmp_echo_time = {
    .name = "icmp_echo_time",
    .packet_length = 62,
    .probe_type = PROBE_ICMP_ECHO_TIME,
    .port_args =0,
    .probe_context_init = icmp_time_probe_context_init,
    .probe_thread_init = icmp_time_probe_thread_init,
    .probe_make_packet = icmp_time_probe_make_packet,
    .probe_dump_packet = icmp_time_probe_dump_packet,
    .probe_validate_packet = icmp_time_probe_validate_packet,
    .probe_process_packet = icmp_time_probe_process_packet,
