...
static xm_cyclic_group_t groups[] = {{// 2^8 + 1
				   .prime = 257,
				   .known_primroot = 3,
				   .prime_factors = {2},
				   .num_prime_factors = 1},
...
static int check_coprime(uint64_t check, const xm_cyclic_group_t *group)
{
    unsigned i;

	for (i = 0; i < group->num_prime_factors; i++) {
...
static uint32_t find_primroot(xm_cyclic_group_t *group, xm_aesrand_t *aes)
{
	uint32_t candidate =
	    (uint32_t)((xm_aesrand_getword(aes) & 0xFFFFFFFF) % group->prime);
	if (candidate == 0) {
		++candidate;
	}
	...
	uint64_t retv = xm_isomorphism(candidate, group);
	return retv;
}

xm_cyclic_group_t *xm_get_cyclic_group(uint64_t min_size)
{
    unsigned i;
...
xm_cycle_t* xm_make_cycle(xm_pool_t *mp,xm_cyclic_group_t *group, xm_aesrand_t *aes)
{
	xm_cycle_t* cycle = (xm_cycle_t*)xm_palloc(mp,sizeof(*cycle));

	cycle->group = group;
	cycle->generator = find_primroot(group, aes);
	cycle->offset = (uint32_t)(xm_aesrand_getword(aes) & 0xFFFFFFFF);
...
uint64_t xm_isomorphism(uint64_t additive_elt, xm_cyclic_group_t *mult_group)
{
