...
static void * _mmap_address_get(xm_shm_interface_t *shm_int,uint64_t offset){

    void *addr = NULL;
    xm_shm_mmap_t *shm_mmap = (xm_shm_mmap_t*)shm_int;
...
static void _mmap_address_free(xm_shm_interface_t *shm_int,void *address){
...
static inline void * _mmap_header_attach(int fd,uint64_t page_size,uint64_t priv_data_size){
	...
	size_t sz = xm_align_up(sizeof(xm_shm_header_t)+priv_data_size,page_size);
...
xm_shm_interface_t * xm_shm_mmap_create(xm_pool_t *mp,const char *fname,uint64_t fsize,uint64_t  entry_size,uint64_t priv_data_size,int is_write){
   
   void *address = NULL;
   xm_shm_mmap_t *shm_mmap = NULL;
   ...
   if(existed == 0 && is_write == 0){
          ...
          xm_log(XM_LOG_ERR,"File:%s not existed,cannot open for mmap and read!",fname);
          return NULL;
  }

  r_fsize = (uint64_t)xm_align_up(fsize,pg_size);
  ...
  if(is_write){
     ...
     if(fd <0){
        xm_log(XM_LOG_ERR,"Cannot open file:%s write by mmap!",fname);
        return NULL;
     }
  }else{
     fd = _mmap_file_open_for_read(fname);
     if(fd <0){
        xm_log(XM_LOG_ERR,"Cannot open file:%s read by mmap!",fname);
        return NULL;
      }
  }

  shm_mmap = (xm_shm_mmap_t*)xm_palloc(mp,sizeof(xm_shm_mmap_t));
  ...
  if(address == NULL){
     xm_log(XM_LOG_ERR,"Cannot attach mmap address!");
     close(fd);
     return NULL;
  }
 
  xm_shm_init((xm_shm_interface_t*)shm_mmap,address,
    r_fsize,r_entry_size,r_priv_data_size,pg_size,is_write);

  /*ok*/
  return (xm_shm_interface_t*)shm_mmap;

}

void xm_shm_mmap_destroy(xm_shm_interface_t *shm_int){
   ...
   if(shm_int->is_write == 0){
      xm_log(XM_LOG_ERR,"Cao,Reader process can not destroy shm!");
