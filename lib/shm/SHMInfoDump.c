...
static void _dump_info(xm_shm_format_t *shm_fmt,int index){

    xm_shm_header_t *shm_int = shm_fmt->shm_int->shm_header;
...
static void shm_close(xm_shm_format_t *shm_fmt){

     xm_shm_mmap_t *shm_mmap = (xm_shm_mmap_t*)shm_fmt->shm_int;
...
int  main(int argc,char **argv){
    ...
    xm_shm_format_t tmp,*shm_fmt=&tmp;
    ...
    xm_pool_t *mp = xm_pool_create(1024);
    if(mp == NULL){
    ...
    shm_fmt = xm_shm_format_with_mmap_create(mp,mmap_fname,0,0,0,0);
