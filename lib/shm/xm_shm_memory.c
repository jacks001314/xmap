...
static void * _memory_address_get(xm_shm_interface_t *shm_int,uint64_t offset){

   xm_shm_memory_t *shm_mem = (xm_shm_memory_t*)shm_int;

   return shm_mem->start_address+offset;
}

static void _memory_address_free(xm_shm_interface_t *shm_int,void *address){
...
static int _del_existed_shm(key_t shm_key)
{
   ...
   xm_log(XM_LOG_ERR,"Cannot remove existed share memory!");

   return -1;
}

/*create a new share memory to write */
static int _create_shm_memory(xm_shm_memory_t *shm_mem,uint64_t size)
{
    int  id;
    key_t shm_key;
    /*get key*/
    ...
    if(shm_key==-1)
    {
	xm_log(XM_LOG_ERR,"Cannot create a share memory key:%s",shm_mem->key);
	return -1;
    }
    ...
    if (id == -1) {
        xm_log(XM_LOG_ERR,"Cannot create a share memory for key:%s",shm_mem->key);
        return -1;
    }
    ...
    {
        xm_log(XM_LOG_ERR,"Cannot attach a share memory for key:%s",shm_mem->key);
	return -1;
    }
...
static int _open_shm_memory(xm_shm_memory_t *shm_mem)
{
    int  id;
    key_t shm_key;
    /*get key*/
    ...
    if (id == -1) {
    	xm_log(XM_LOG_ERR,"Cannot open share memory to read ,key:%s",shm_mem->key);
        return -1;
    }
...
xm_shm_interface_t * xm_shm_memory_create(xm_pool_t *mp,const char *key,int proj_id,uint64_t size,uint64_t  entry_size,uint64_t priv_data_size,int is_write){
  ...
  r_size = (uint64_t)xm_align_up(size,pg_size);
  r_entry_size = (uint64_t)xm_align_up(entry_size,pg_size);

  if(priv_data_size!=0)
     r_priv_data_size = (uint64_t)xm_align_up(priv_data_size,pg_size);

  shm_mem = (xm_shm_memory_t*)xm_palloc(mp,sizeof(xm_shm_memory_t));
  shm_mem->key = key;
  ...
  if(is_write){
     ...
     if(rc <0){
        xm_log(XM_LOG_ERR,"Cannot create a share memory to write for key:%s!",key);
        return NULL;
     }
  }else{
     rc = _open_shm_memory(shm_mem);
     if(rc <0){
        xm_log(XM_LOG_ERR,"Cannot open share memory to read for key:%s!",key);
        return NULL;
      }
  }
  ...
  xm_shm_init((xm_shm_interface_t*)shm_mem,shm_mem->start_address,
    r_size,r_entry_size,r_priv_data_size,pg_size,is_write);

  /*ok*/
  return (xm_shm_interface_t*)shm_mem;

}

void xm_shm_memory_destroy(xm_shm_interface_t *shm_int){
