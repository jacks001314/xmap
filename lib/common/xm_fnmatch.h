...
#include "xm_tables.h"
/**
 * @defgroup xm_fnmatch Filename Matching Functions
 * @ingroup GW 
 * @{
 */

#define XM_FNM_NOMATCH     1     /**< Match failed. */
 
#define XM_FNM_NOESCAPE    0x01  /**< Disable backslash escaping. */
#define XM_FNM_PATHNAME    0x02  /**< Slash must be matched by slash. */
#define XM_FNM_PERIOD      0x04  /**< Period must be matched by period. */
#define XM_FNM_CASE_BLIND  0x08  /**< Compare characters case-insensitively. */

/**
 * Try to match the string to the given pattern, return 0 if
 *    match, else return XM_FNM_NOMATCH.  Note that there is no such thing as
 *    an illegal pattern.
 *
 ...
 * <pre>
 *              XM_FNM_NOESCAPE       Disable backslash escaping
 *              XM_FNM_PATHNAME       Slash must be matched by slash
 *              XM_FNM_PERIOD         Period must be matched by period
 *              XM_FNM_CASE_BLIND     Compare characters case-insensitively.
 * </pre>
 */

int xm_fnmatch(const char *pattern,const char *strings, int flags);
 ...
 */
int xm_fnmatch_test(const char *pattern);
 ...
 * returned.
 */
int xm_match_glob(const char *dir_pattern, 
                                         xm_array_header_t **result,
                                         xm_pool_t *p);
