...
static char *xm_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    double fi, fj;
...
static char *xm_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (xm_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *xm_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (xm_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * xm_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *xm_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = xm_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
    ...
    char *buf, size_t *len)
{
    ...
    if (format == 'f')
        p = xm_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = xm_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
 ...
 */
int xm_vformatter(int (*flush_func)(xm_vformatter_buff_t *),
    xm_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        if (*fmt != '%') {
            ...
            /*
             * Modifier check.  In same cases, XM_OFF_T_FMT can be
             * "lld" and XM_INT64_T_FMT can be "ld" (that is, off_t is
             * "larger" than int64). Check that case 1st.
             * Note that if XM_OFF_T_FMT is "d",
             * the first if condition is never true. If XM_INT64_T_FMT
             * is "d' then the second if condition is never true.
             */
            if ((sizeof(XM_OFF_T_FMT) > sizeof(XM_INT64_T_FMT)) &&
                ((sizeof(XM_OFF_T_FMT) == 4 &&
                 fmt[0] == XM_OFF_T_FMT[0] &&
                 fmt[1] == XM_OFF_T_FMT[1]) ||
                (sizeof(XM_OFF_T_FMT) == 3 &&
                 fmt[0] == XM_OFF_T_FMT[0]) ||
                (sizeof(XM_OFF_T_FMT) > 4 &&
                 strncmp(fmt, XM_OFF_T_FMT, 
                         sizeof(XM_OFF_T_FMT) - 2) == 0))) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(XM_OFF_T_FMT) - 2);
            }
            else if ((sizeof(XM_INT64_T_FMT) == 4 &&
                 fmt[0] == XM_INT64_T_FMT[0] &&
                 fmt[1] == XM_INT64_T_FMT[1]) ||
                (sizeof(XM_INT64_T_FMT) == 3 &&
                 fmt[0] == XM_INT64_T_FMT[0]) ||
                (sizeof(XM_INT64_T_FMT) > 4 &&
                 strncmp(fmt, XM_INT64_T_FMT, 
                         sizeof(XM_INT64_T_FMT) - 2) == 0)) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(XM_INT64_T_FMT) - 2);
            }
            else if (*fmt == 'q') {
                var_type = IS_QUAD;
                fmt++;
            ...
            case 'G':
                 ...
                 * * We use &num_buf[ 1 ], so that we have room for the sign
                 */
                s = xm_gcvt(va_arg(ap, double), (int) precision, &num_buf[1],
                            alternate_form);
                if (*s == '-')
                    prefix_char = *s++;
                else if (print_sign)
...
static int snprintf_flush(xm_vformatter_buff_t *vbuff)
{
    /* if the buffer fills we have to abort immediately, there is no way
     * to "flush" an xm_snprintf... there's nowhere to flush it to.
     */
	vbuff = vbuff;
...
int xm_snprintf(char *buf, size_t len, 
                                     const char *format, ...)
{
    int cc;
    va_list ap;
    ...
    va_start(ap, format);
    cc = xm_vformatter(snprintf_flush, &vbuff, format, ap);
    va_end(ap);
    if (len != 0) {
        *vbuff.curpos = '\0';
    }
...
int xm_vsnprintf(char *buf, size_t len, const char *format,
                               va_list ap)
{
    int cc;
    xm_vformatter_buff_t vbuff;
...
static int psprintf_flush(xm_vformatter_buff_t *vbuff)
{
...
char * xm_pvsprintf(xm_pool_t *pool, const char *fmt, va_list ap)
{
    ...
    if (xm_vformatter(psprintf_flush, &ps.vbuff, fmt, ap) == -1) {
