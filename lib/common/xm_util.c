 ...
 */
char *xm_utf8_unicode_inplace_ex(xm_pool_t *mp, unsigned char *input, long int input_len, int *changed) {
    int unicode_len = 0;
        ...
        /* If first byte begins with binary 110 it is two byte encoding*/
            ...
            /* check second byte starts with binary 10 */
                ...
                if(count <= len) {
                    ...
                    *data++ = '%';
                    *data++ = 'u';
                    unicode = (unsigned char*)xm_psprintf(mp, "%x", d);
 ...
 */
unsigned char xm_is_netmask_v4(char *ip_strv4) {
 ...
 */
unsigned char xm_is_netmask_v6(char *ip_strv6) {
 ...
 */
char *xm_remove_quotes(xm_pool_t *mptmp, const char *input, int input_len)  {
    char *parm = xm_palloc(mptmp, input_len);
    char *ret = parm;
    int len = input_len;
 ...
 * \retval string On Success
 */
char *xm_remove_escape(xm_pool_t *mptmp, const char *input, int input_len)  {
    char *parm = xm_palloc(mptmp, input_len);
    char *ret = parm;
    int len = input_len;
 ...
 *
 */
int xm_parse_boolean(const char *input) {
 ...
 */
int xm_decode_base64_ext(char *plain_text, const unsigned char *input, int input_len)   {
 ...
 */
int xm_convert_to_int(const char c)
{
    int n;
    if ((c>='0') && (c<='9'))
        n = c - '0';
 ...
 */
int xm_parse_name_eq_value(xm_pool_t *mp, const char *input, char **name, char **value) {
    char *p = NULL;
    ...
    *name = xm_pstrmemdup(mp, input, p - input);
    if (*name == NULL) return -1;
    p++;

    *value = xm_pstrdup(mp, p);
 ...
 */
char *xm_url_encode(xm_pool_t *mp, char *input, unsigned int input_len, int *changed) {
    char *rval, *d;
    unsigned int i, len;
 ...
 *
 */
char *xm_file_basename(xm_pool_t *mp, const char *filename) {
    char *d, *p;

    if (filename == NULL) return NULL;
    d = xm_pstrdup(mp, filename);
...
char *xm_file_dirname(xm_pool_t *p, const char *filename) {
    char *b, *c;

    if (filename == NULL) return NULL;
    b = xm_pstrdup(p, filename);
 ...
 *
 */
int xm_sql_hex2bytes_inplace(unsigned char *data, int len) {
 ...
 *
 */
int xm_hex2bytes_inplace(unsigned char *data, int len) {
 ...
 * representation.
 */
char *xm_bytes2hex(xm_pool_t *pool, unsigned char *data, int len) {
    static const unsigned char b2hex[] = "0123456789abcdef";
    char *hex = NULL;
    int i, j;

    hex = xm_palloc(pool, (len * 2) + 1);
 ...
 * representation.
 */
char *xm_bytes2hex_malloc(unsigned char *data, int len) {
    static const unsigned char b2hex[] = "0123456789abcdef";
    char *hex = NULL;
    int i, j;
 ...
 *
 */
int xm_is_token_char(unsigned char c) {
 ...
 *
 */
int xm_remove_lf_crlf_inplace(char *text) {
    char *p = text;
    int count = 0;
 ...
 *
 */
char *xm_guess_tmp_dir(xm_pool_t *p) {
    ...
    /* ENH Use xm_temp_dir_get instead. */
 ...
 *
 */
char *xm_current_logtime(xm_pool_t *mp) {
    xm_time_exp_t t;
    char tstr[100];
    size_t len;

    xm_time_exp_lt(&t, xm_time_now());

    xm_strftime(tstr, &len, 80, "%d/%b/%Y:%H:%M:%S ", &t);
    xm_snprintf(tstr + strlen(tstr), 80 - strlen(tstr), "%c%.2d%.2d",
            t.tm_gmtoff < 0 ? '-' : '+',
            t.tm_gmtoff / (60 * 60), (t.tm_gmtoff / 60) % 60);
    return xm_pstrdup(mp, tstr);
}

char  *xm_current_logtime_with_buf(char *buf,size_t buf_size){
    ...
    xm_strftime(buf, &len, 80, "%d/%b/%Y:%H:%M:%S ", &t);

    xm_snprintf(buf + strlen(buf), 80 - strlen(buf), "%c%.2d%.2d",
            t.tm_gmtoff < 0 ? '-' : '+',
            t.tm_gmtoff / (60 * 60), (t.tm_gmtoff / 60) % 60);
 ...
 *
 */
char *xm_current_filetime(xm_pool_t *mp) {
    xm_time_exp_t t;
    char tstr[100];
    size_t len;
    ...
    xm_strftime(tstr, &len, 80, "%Y%m%d-%H%M%S", &t);
    return xm_pstrdup(mp, tstr);
}
 ...
 *
 */
int xm_mkstemp_ex(char *templat, int mode) {
    int fd = -1;

    /* ENH Use xm_file_mktemp instead. */
 ...
 *
 */
int xm_mkstemp(char *templat) {
    return xm_mkstemp_ex(templat, CREATEMODE_UNISTD);
}

/**
 * Converts the input string to lowercase (in-place).
 */
char *xm_strtolower_inplace(unsigned char *str) {
    unsigned char *c = str;
...
static char *_log_escape(xm_pool_t *p, const unsigned char *input,
        unsigned long int input_length, int escape_quotes, int escape_colon, int escape_re);

char *xm_log_escape_re(xm_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 1, 1, 1);
}

char *xm_log_escape(xm_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 1, 0, 0);
}

char *xm_log_escape_nq(xm_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 0, 0, 0);
}

char *xm_log_escape_ex(xm_pool_t *mp, const char *text, unsigned long int text_length) {
    return _log_escape(mp, (const unsigned char *)text, text_length, 1, 0, 0);
}

char *xm_log_escape_nq_ex(xm_pool_t *mp, const char *text, unsigned long int text_length) {
    return _log_escape(mp, (const unsigned char *)text, text_length, 0, 0, 0);
}

char *xm_log_escape_raw(xm_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = xm_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;

    for (i = 0, j = 0; i < text_length; i++, j += 4) {
        ret[j] = '\\';
        ret[j+1] = 'x';
        xm_c2x(text[i], ret+j+2);
    }
    ret[text_length * 4] = '\0';
...
char *xm_log_escape_nul(xm_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = xm_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;
 ...
 */
char *xm_log_escape_hex(xm_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = xm_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;
 ...
 */
static char *_log_escape(xm_pool_t *mp, const unsigned char *input, unsigned long int input_len,
        int escape_quotes, int escape_colon, int escape_re)
{
    ...
    ret = xm_palloc(mp, input_len * 4 + 1);
...
int xm_js_decode_nonstrict_inplace(unsigned char *input, long int input_len) {
 ...
 * IMP1 Assumes NUL-terminated
 */
int xm_urldecode_nonstrict_inplace_ex(unsigned char *input, long int input_len, int *invalid_count, int *changed) {
    unsigned char *d = (unsigned char *)input;
    long int i, count;
 ...
 * IMP1 Assumes NUL-terminated
 */
int xm_html_entities_decode_inplace(xm_pool_t *mp, unsigned char *input, int input_len) {
            ...
            if (input[j] == '#') {
                ...
                if ((input[j] == 'x')||(input[j] == 'X')) {
                    ...
                    if (j > k) { /* Do we have at least one digit? */
                        /* Decode the entity. */
                        char *x = xm_pstrmemdup(mp, (const char *)&input[k], j - k);
 ...
 * IMP1 Assumes NUL-terminated
 */
int xm_ansi_c_sequences_decode_inplace(unsigned char *input, int input_len) {
    unsigned char *d = input;
    int i, count;
 ...
 * IMP1 Assumes NUL-terminated
 */
int xm_normalize_path_inplace(unsigned char *input, int input_len, int win, int *changed) {
    unsigned char *src;
    unsigned char *dst;
    unsigned char *end;
    int ldst = 0;
...
char *xm_resolve_relative_path(xm_pool_t *pool, const char *parent_filename, const char *filename) {
    if (filename == NULL) return NULL;
    // TODO Support paths on operating systems other than Unix.
    if (filename[0] == '/') return (char *)filename;

    return xm_pstrcat(pool, xm_pstrndup(pool, parent_filename,
                strlen(parent_filename) - strlen(xm_filepath_name_get(parent_filename))),
            filename, NULL);
}
 ...
 */
int xm_css_decode_inplace(unsigned char *input, long int input_len) {
    unsigned char *d = (unsigned char *)input;
    long int i, j, count;
            ...
            if (i + 1 < input_len) {
                ...
                if (j > 0) { /* We have at least one valid hexadecimal character. */
                    ...
                    switch (j) {
                        ...
                        case 1:
                            *d++ = xm_xsingle2c(&input[i]);
                            break;

                        case 2:
                        case 3:
                            /* Use the last two from the end. */
                            *d++ = xm_x2c(&input[i + j - 2]);
                            break;
...
/**
 * @brief Transforms an xm_array_header_t to a text buffer
 *
 * Converts an xm_array_header_t into a plain/text buffer in a Key: Pair
 ...
 *
 * @param arr xm_array_header_t to be iterated.
 ...
 *
 */
int xm_headers_to_buffer(const xm_array_header_t *arr, char *buffer,
        int buffer_length)
{
    int headers_length = 0;
    int write_to_buffer = 0;
    int i = 0;
    const xm_table_entry_t *te = NULL;

    if (buffer != NULL && buffer_length > 0) {
        write_to_buffer = 1;
    }

    te = (xm_table_entry_t *)arr->elts;
...
int xm_read_line(char *buf, int len, FILE *fp)
{
    char *tmp;
...
unsigned long xm_get_current_timems(void)
{
   struct timeval tv;
...
unsigned char* xm_encode_base64(xm_pool_t *mp,const unsigned char *input,int input_len){

	size_t dst_len = BASE64_LEN(input_len);

	unsigned char *dst = (unsigned char*)xm_pcalloc(mp,dst_len);
...
unsigned char* xm_encode_base64_malloc(const unsigned char *input,int input_len){
