...
static xm_inline int fnmatch_ch(const char **pattern, const char **string, int flags)
{
    const char * const mismatch = *pattern;
    const int nocase = !!(flags & XM_FNM_CASE_BLIND);
    const int escape = !(flags & XM_FNM_NOESCAPE);
    const int slash = !!(flags & XM_FNM_PATHNAME);
    int result = XM_FNM_NOMATCH;
    const char *startch;
    int negate;
...
leadingclosebrace:
         ...
         */
        *pattern = mismatch;
        result = XM_FNM_NOMATCH;
    }
    else if (**pattern == '?') {
        /* Optimize '?' match before unescaping **pattern */
        if (!**string || (slash && (**string == '/')))
            return XM_FNM_NOMATCH;
        result = 0;
...
int xm_fnmatch(const char *pattern, const char *string, int flags)
{
        ...
        while (*pattern)
        {
            ...
            if (wild)
            {
                ...
                for (matchptr = pattern, matchlen = 0; 1; ++matchlen)
                {
                     ...
                     * Use a dummy fnmatxm_ch() test to count one "[range]" escape
                     */ 
                    /* XXX: Adjust for MBCS */
...
int xm_match_glob(const char *pattern, 
                                         ...
                                         xm_pool_t *p)
{
     ...
     * we really need xm_filepath_basename, which will be coming as soon as
     * I get to it.  rbb
     */
    ...
    if (idx == NULL) {
        ...
        path = xm_pstrndup(p, pattern, idx - pattern);
        pattern = idx + 1;
    }

    *result = xm_array_make(p, 0, sizeof(char *));
    dir = opendir(path);
    ...
    while ((next = readdir(dir)) != NULL) {
        if (xm_fnmatch(pattern, next->d_name, 0) == 0) {
            *(const char **)xm_array_push(*result) = xm_pstrdup(p, next->d_name);
        }
    }
