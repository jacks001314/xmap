...
char * xm_cpystrn(char *dst, const char *src, size_t dst_size)
{
 ...
 * be expanded to include support for the xm_call_exec command line
...
int xm_tokenize_to_argv(const char *arg_str, 
                                            char ***argv_out,
...
#define REMOVE_ESCAPE_CHARS(cleaned, dirty, escaped) \
    ...
    *argv_out = xm_palloc(token_context, numargs * sizeof(char*));
    ...
    for (argnum = 0; argnum < (numargs-1); argnum++) {
        ...
        cp++;
        (*argv_out)[argnum] = xm_palloc(token_context, cp - ct);
        xm_cpystrn((*argv_out)[argnum], ct, cp - ct);
...
const char * xm_filepath_name_get(const char *pathname)
{
 ...
 */
char * xm_collapse_spaces(char *dest, const char *src)
{
    while (*src) {
        if (!xm_isspace(*src)) 
            *dest++ = *src;
