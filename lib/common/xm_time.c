...
int xm_time_ansi_put(xm_time_t *result,
                                            time_t input)
{
    *result = (xm_time_t)input * XM_USEC_PER_SEC;
    return 0;
}
...
xm_time_t xm_time_now(void)
{
    ...
    return tv.tv_sec * XM_USEC_PER_SEC + tv.tv_usec;
}

static void explode_time(xm_time_exp_t *xt, xm_time_t t,
                         int32_t offset, int use_localtime)
{
    struct tm tm;
    time_t tt = (t / XM_USEC_PER_SEC) + offset;
    xt->tm_usec = t % XM_USEC_PER_SEC;
...
int xm_time_exp_gmt(xm_time_exp_t *result,
                                           xm_time_t input)
{
    return xm_time_exp_tz(result, input, 0);
}
                                                ...
                                                xm_time_t input)
{
    ...
    return xm_time_exp_tz(result, input, -timezone);
#else
...
int xm_time_exp_get(xm_time_t *t, xm_time_exp_t *xt)
{
    xm_time_t year = xt->tm_year;
    xm_time_t days;
    ...
    *t = days * XM_USEC_PER_SEC + xt->tm_usec;
    return 0;
}

int xm_time_exp_gmt_get(xm_time_t *t, 
                                               xm_time_exp_t *xt)
{
    int status = xm_time_exp_get(t, xt);
    if (status == 0)
        *t -= (xm_time_t) xt->tm_gmtoff * XM_USEC_PER_SEC;
    return status;
}
...
{
    struct timeval tv;
    tv.tv_usec = t % XM_USEC_PER_SEC;
    tv.tv_sec = t / XM_USEC_PER_SEC;
...
#if XM_HAS_THREADS && defined(_POSIX_THREAD_SAFE_FUNCTIONS)
    gmtime_r(&t1, &t);
#else
    t = *gmtime(&t1);
#endif
