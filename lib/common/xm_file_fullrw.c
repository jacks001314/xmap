                                             ...
                                             size_t *bytes_read)
{
	...
	status = xm_file_read(thefile, buf, &amt);
	buf = (char *)buf + amt;
        nbytes -= amt;
        total_read += amt;
                                              ...
                                              size_t *bytes_written)
{
	...
	status = xm_file_write(thefile, buf, &amt);
	buf = (char *)buf + amt;
        nbytes -= amt;
        total_written += amt;
    } while (status == 0 && nbytes > 0);
                                               ...
                                               size_t *bytes_written)
{
    ...
    rv = xm_file_writev(thefile, vec, nvec, &amt);
    ...
    if (amt) {
        rv = xm_file_write_full(thefile, (const char *)vec[i].iov_base + amt,
                                 vec[i].iov_len - amt, NULL);
    }

    for (; i < nvec && rv == 0; i++) {
        rv = xm_file_write_full(thefile, vec[i].iov_base,
                                 vec[i].iov_len, &amt);
    }
