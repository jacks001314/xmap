...
{
    {"http",     XM_URI_HTTP_DEFAULT_PORT},
    {"ftp",      XM_URI_FTP_DEFAULT_PORT},
    {"https",    XM_URI_HTTPS_DEFAULT_PORT},
    {"gopher",   XM_URI_GOPHER_DEFAULT_PORT},
    {"ldap",     XM_URI_LDAP_DEFAULT_PORT},
    {"nntp",     XM_URI_NNTP_DEFAULT_PORT},
    {"snews",    XM_URI_SNEWS_DEFAULT_PORT},
    {"imap",     XM_URI_IMAP_DEFAULT_PORT},
    {"pop",      XM_URI_POP_DEFAULT_PORT},
    {"sip",      XM_URI_SIP_DEFAULT_PORT},
    {"rtsp",     XM_URI_RTSP_DEFAULT_PORT},
    {"wais",     XM_URI_WAIS_DEFAULT_PORT},
    {"z39.50r",  XM_URI_WAIS_DEFAULT_PORT},
    {"z39.50s",  XM_URI_WAIS_DEFAULT_PORT},
    {"prospero", XM_URI_PROSPERO_DEFAULT_PORT},
    {"nfs",      XM_URI_NFS_DEFAULT_PORT},
    {"tip",      XM_URI_TIP_DEFAULT_PORT},
    {"acap",     XM_URI_ACAP_DEFAULT_PORT},
    {"telnet",   XM_URI_TELNET_DEFAULT_PORT},
    {"ssh",      XM_URI_SSH_DEFAULT_PORT},
...
/* We have a xm_table_t that we can index by character and it tells us if the
 ...
 */
int xm_uri_parse(xm_pool_t *p, const char *uri, 
                                        xm_uri_t *uptr)
{
    const char *s;
    const char *s1;
...
deal_with_path:
        ...
        if (s != uri) {
            uptr->path = xm_pstrmemdup(p, uri, s - uri);
        ...
        if (*s == '?') {
            ...
            s1 = strchr(s, '#');
            if (s1) {
                uptr->fragment = xm_pstrdup(p, s1 + 1);
                uptr->query = xm_pstrmemdup(p, s, s1 - s);
            }
            else {
                uptr->query = xm_pstrdup(p, s);
            }
            return 0;
        }
        /* otherwise it's a fragment */
        uptr->fragment = xm_pstrdup(p, s + 1);
        return 0;
    }
    ...
    if (s != uri && s[0] == ':') {
        uptr->scheme = xm_pstrmemdup(p, uri, s - uri);
        s++;
    }
    else {
...
deal_with_authority:
    ...
    uptr->hostinfo = xm_pstrmemdup(p, hostinfo, uri - hostinfo);
...
deal_with_host:
        ...
        ++s;
        uptr->port_str = xm_pstrmemdup(p, s, uri - s);
        if (uri != s) {
        ...
        uptr->port = xm_uri_port_of_scheme(uptr->scheme);
        goto deal_with_path;
    }
    ...
    if (s1) {
        uptr->user = xm_pstrmemdup(p, hostinfo, s1 - hostinfo);
        ++s1;
        uptr->password = xm_pstrmemdup(p, s1, s - s1);
    }
    else {
        uptr->user = xm_pstrmemdup(p, hostinfo, s - hostinfo);
    }
    hostinfo = s + 1;
    goto deal_with_host;
}
