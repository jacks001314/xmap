...
static inline int _new_memory(xm_object_pool_t *omp){

	xm_object_mem_t *objm;
	xm_object_item_t *item;
	...
	objm = (xm_object_mem_t*)addr;

	pos = (void*)(objm+1);
	end = addr+OBJECT_MEM_SIZE;

	/*alloc object item and add to free list*/
	r_item_size = omp->object_size+sizeof(xm_object_item_t);

	while(pos+r_item_size<end){
	

		item = (xm_object_item_t*)pos;
...
xm_object_pool_t * xm_object_pool_create(xm_pool_t *mp,size_t object_limits,
	size_t object_size,
	void *priv_data,
	...
	if(object_size>=OBJECT_MEM_SIZE){
		...
		xm_log(XM_LOG_ERR,"The object size is too large:%lu",object_size);
		return NULL;
	}

	xm_object_pool_t *objp = (xm_object_pool_t *) xm_pcalloc(mp,sizeof(*objp));
...
void xm_object_pool_destroy(xm_object_pool_t *omp){

	xm_object_mem_t *objm = omp->mm_list;
	xm_object_mem_t *prev;
...
void * xm_object_pool_get(xm_object_pool_t *omp){
	...
	if(list_empty(&omp->free_list)){
		...
		if(omp->object_limits&&omp->n_objects>omp->object_limits){
			...
			xm_log(XM_LOG_ERR,"Too many object has been alloced!");
			return NULL;
		}

		if(_new_memory(omp)){

			xm_log(XM_LOG_ERR,"No memory space to  new object!");
			return NULL;
		}
	}

	item = list_first_entry(&omp->free_list,xm_object_item_t,node);
	list_del(&item->node);
...
void  xm_object_pool_put(xm_object_pool_t *omp,void *obj){

	if(obj == NULL)
		return;

	xm_object_item_t *item = (xm_object_item_t*)(obj-sizeof(*item));
...
#define ROBJ_SIZE(omp,n) (unsigned long)(((omp)->object_size+sizeof(xm_object_item_t))*(n))

void xm_object_pool_dump(xm_object_pool_t *omp,FILE *fp){


	fprintf(fp,"Dump object pool info-------------------------------------------\n");
	
	fprintf(fp,"The object size:%lu\n",(unsigned long)(omp->object_size+sizeof(xm_object_item_t)));
