/*
 *
 *      Filename: test_pfring.c
 *
 *        Author: shajf,csp001314@163.com
 *   Description: ---
 *        Create: 2017-02-23 18:06:23
 * Last Modified: 2017-02-27 17:39:00
 */
...
int main(int argc,char ** argv){
	...
	if(argc!=5){
		...
		fprintf(stderr,"usage: main_name <task number> <core_mask> <interface_name> <cfname>\n");
		return -1;
	}
	
	tsk_number = (uint32_t)atoi(argv[1]);
	core_mask = argv[2];
	interface.if_name = argv[3];
	interface.if_cfname = argv[4];
	...
	driver = xm_if_pfring_create(mp,&interface);
	if(driver == NULL){
	
		fprintf(stderr,"create pfring driver failed!\n");
		return -1;
	}
